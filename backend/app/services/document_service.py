...
def build_retrieval_plan(query: str) -> RetrievalPlan:
    ...
    if any(token in lowered for token in ["어떤 질문", "질문 추천", "무슨 질문", "뭘 물어", "뭐 물어"]):
        return RetrievalPlan(strategy="keyword", top_k=3)

    if any(token in lowered for token in ["비교", "차이", "다른 점", "한 번에", "모두", "같나요"]):
        return RetrievalPlan(
            strategy="mmr",
            top_k=6,
            files=_merge_files(COURSE_FILES, PLAYDATA_FILES),
        )

    if any(
        token in lowered
        for token in ["교육비", "수강료", "훈련장려금", "본인부담금", "지원금", "얼마", "유효기간", "출결"]
    ):
        return RetrievalPlan(
            strategy="hybrid",
            top_k=6,
    ...
    if any(token in lowered for token in ["비전공자", "어떤 사람", "잘 맞", "추천 대상", "취업", "직무", "과정"]):
        return RetrievalPlan(
            strategy="hybrid",
            top_k=6,
            files=_merge_files(COURSE_FILES, PLAYDATA_FILES),
