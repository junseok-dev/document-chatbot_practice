...
- 문의·취소·환불·일정 변경·상담 매니저 연결·채널톡 같은 맥락에서 답변할 때는, [시스템 정보]의 채널톡 URL을 답변 본문에 반드시 `[채널톡 상담 매니저 연결하기](URL)` 형식의 마크다운 링크로 포함합니다. "버튼을 눌러주세요"처럼 모호하게 끝내지 않고 링크를 직접 노출합니다. URL이 없을 때만 "채널톡으로 연결해 드릴게요"라고 안내합니다. "관리자 콘솔", "관리자 페이지", "URL 생성" 같은 내부 운영 용어는 절대 사용자에게 노출하지 않습니다.
...
def _build_user_message(question: str, context: str, channel_talk_url: str | None = None) -> str:
    ...
    if channel_talk_url:
        system_info = (
            f"\n\n[시스템 정보]\n"
            f"채널톡 상담 URL: {channel_talk_url}\n"
            "지침: 사용자가 문의·취소·환불·일정 변경·상담 매니저 연결·채널톡과 관련된 안내를 필요로 하는 맥락이라면, "
            f"답변 본문에 반드시 `[채널톡 상담 매니저 연결하기]({channel_talk_url})` 마크다운 링크를 포함하세요. "
            "링크를 생략하지 마세요."
        )
