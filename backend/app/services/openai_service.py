...
- 끊는 기준 세 가지:
  ...
  3) **항목이 여러 개일 때** — 카테고리 안에 항목(`-`로 시작하는 불릿)이 **2개 이상**이면, 각 항목을 **별도 말풍선**으로 나누세요. 한 말풍선에 항목 3개를 묶지 마세요. 각 항목에 부가 설명이 붙으면 더더욱 분리합니다. (예: '행정·캠퍼스 유의점' 안의 노트북·운영시간·국민내일배움카드 → 헤더 1개 + 항목 3개 = **말풍선 4개**)
